...
# Ensure you have set the right working directory pointing to the folder in which you have your RIS file(s)
getwd()
...
# See acroos all available models
...
performance <- 
  ...
  AIscreenR::screen_analyzer(
    # Name variable with human decisions
    human_decision = human_code,
    # State whether only key results should be printed
    key_result = TRUE # Default
  )

performance 

# Use multiple screenings
plan(multisession)
...
# Get a report with screening disaggreements 
report(
...
# Approximate price
# Approximate price full screening (2862 refs)
